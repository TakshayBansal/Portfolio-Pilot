...
def get_market_trend(ticker: str = 'SPY', short_window: int = 50, long_window: int = 200) -> str:
    """
    Determines the market trend based on short-term and long-term moving averages.

    Args:
        ticker (str): The ticker symbol for the market index (default 'SPY').
...
if __name__ == '__main__':
    # Example usage:
    # Ensure yfinance is installed and data_loader can fetch SPY.
    ...
    try:
        ...
        # from data_loader import load_data 
        print(f"Current market trend for SPY: {get_market_trend('SPY')}")
